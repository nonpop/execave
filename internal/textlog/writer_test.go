...
func TestFormatEntry_DenyReadAbsPath_FormatsColumnsAndShortensPath(t *testing.T) {
	w := New(nil, "/home/user", "/home/user/project", false, false, nil, nil)
	...
	assert.Equal(t, "DENY    READ   ~/.ssh/id_rsa  (no-matching-rule)", w.formatEntry(entry))
}
...
func TestFormatEntry_OKWrite_FormatsColumnsAndShortensPath(t *testing.T) {
	...
	assert.Equal(t, "OK      WRITE  out.txt  (fs:rw:~/project)", w.formatEntry(entry))
}
...
func TestFormatEntry_HTTP_TargetNotShortened(t *testing.T) {
	...
	assert.Equal(t, "OK      HTTP   api.example.com:443  (net:http:api.example.com:443)", w.formatEntry(entry))
}

func TestFormatEntry_UnknownResult_FormatsCorrectly(t *testing.T) {
	w := New(nil, "", "", false, false, nil, nil)
	entry := accesslog.Entry{
		Operation: accesslog.OperationRead,
	...
	assert.Equal(t, "UNKNOWN READ   /relative/path  (unresolved-relative-path)", w.formatEntry(entry))
}
...
func TestFormatEntry_EmptyHomeDirAndConfigDir_UsesAbsolutePath(t *testing.T) {
	...
	assert.Equal(t, "OK      READ   /usr/bin/cat  (fs:ro:/usr)", w.formatEntry(entry))
}
