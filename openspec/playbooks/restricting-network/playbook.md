...
The user runs commands inside the sandbox with network access controlled by rules. Rules grant HTTP or no-access permissions on specific endpoints. A forward proxy always runs; without net rules it is deny-all. Network traffic flows through the proxy, which enforces the allowlist.
