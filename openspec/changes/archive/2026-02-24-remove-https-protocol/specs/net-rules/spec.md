...
### Requirement: Protocol matching
**Reason**: The previous protocol matching distinguished between HTTPS and HTTP rules. Since `https` is no longer a valid action, there is only one allow protocol (`http`) and one deny protocol (`none`). The requirement is replaced by the simplified protocol matching above.
**Migration**: Change `net:https:` rules to `net:http:`.
