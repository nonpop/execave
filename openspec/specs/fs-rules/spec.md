...
`CheckAccess` SHALL return an uncertain result whenever symlink resolution causes the walk to enter a managed path, even if the symlink target itself is not a managed path but remaining path components lead into one.

#### Scenario: Symlink points to ancestor of managed path

- **WHEN** a resolver is created with rules `rw:/mount` and `ro:/managed_parent`, and managed path `/managed_parent/sub`
- **AND** `/mount/link` is a symlink to `/managed_parent`
- **AND** `CheckAccess` is called with path `/mount/link/sub/file` and operation read
- **THEN** the result is uncertain and not allowed
