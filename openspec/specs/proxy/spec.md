...
The proxy capability provides a forward HTTP proxy that enforces network access controls for sandboxed processes. It listens on a Unix domain socket (UDS), handles CONNECT requests for tunneling and plain HTTP requests, validates each request against the network rules allowlist, and integrates with the access log to record network operations.
